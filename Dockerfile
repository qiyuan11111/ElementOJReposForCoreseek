...
RUN mkdir -p /var/cache/yum/x86_64/6/base && mkdir -p /var/cache/yum/x86_64/6/extras && mkdir -p /var/cache/yum/x86_64/6/updates \
	...
	&& cd /coreseek && tar -zxvf /coreseek/coreseek-4.1.tar.gz && cd /coreseek/mmseg-3.2.14 \
	&& ./bootstrap && ./configure --prefix=/usr/local/mmseg3 && make && make install && cd /coreseek/csft-4.1 \
